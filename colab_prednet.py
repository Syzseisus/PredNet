...
class PredNet(nn.Module):
    def __init__(self, R_channels, A_channels, output_mode='error'):
        ...
        if self.output_mode == 'prediction_all': self.prediction_all = []
        elif self.output_mode = 'error_all': self.error_all = []
        elif output_mode == 'out_all':
            self.As = []
            self.Ahats = []
            self.Es = []
            self.Rs = []
        ...
        for i in range(self.n_layers):
            conv = nn.Sequential(nn.Conv2d(self.r_channels[i], self.a_channels[i], 3, padding=1),
                                 nn.ReLU())
            if i == 0:
        ...
        for l in range(self.n_layers - 1):
            update_A = nn.Sequential(nn.Conv2d(2* self.a_channels[l], self.a_channels[l+1],
                                               3, padding=1), 
                                     self.maxpool)
    ...
    def forward(self, input):
        ...
        if self.output_mode == 'error': total_error = []
        ...
        for t in range(time_steps):
            ...
            # Left side: Recurrence deconvolution layers
            ...
            # Right side: standard convolution layers
            for l in range(self.n_layers):
        ...
        if self.output_mode == 'error':
            ...
            return torch.mean(abs(errors - targets).view(-1))
